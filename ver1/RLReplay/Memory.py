...
from Param import MultipleReplayMemoyParam
from SamplingSumTree_py.SamplingSumTree import SamplingSumTree
...
class BaseReplayBuffer:
    ...
    def __init__(self, 
        ...
        self._rewards = torch.empty(tuple([capacity]) + stateShape, dtype=torch.float16, device=self._device)
        self._nextStatus = torch.empty(tuple([capacity]) + stateShape, dtype=stateType,device=self._device)
        self._dones = torch.empty(tuple([capacity]) + stateShape, dtype=torch.int8, device=self._device)
...
class ReplayBuffer(BaseReplayBuffer):
    ...
    def to(self, device: torch.device):
        ...
        self._rewrads = self._rewrads.to(device)
...
class PERBuffer(BaseReplayBuffer):
    ...
    def updatePriorities(self, tdDiffs: Tensor, indices: Tensor):
        newPriorities = self._calcPriorities(tdDiffs).cpu().numpy()
        indicesNP = indices.cpu().numpy()
